...
\item
Weak LLN means that the sample mean \(\overline{Y}_T\) converges in probability to the population mean \(\mu \).
That is, the sequence \(\overline{Y}_{1},\overline{Y}_{2},\ldots \) of random variables
...
\item
Here is an extended illustration for several distributions: 
...
\item
Here is an extended illustration for different error term distributions: 
